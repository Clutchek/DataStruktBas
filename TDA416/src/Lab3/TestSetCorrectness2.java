...
public class TestSetCorrectness2 {
    ...
    private static void testLinkedList(int n2, int n3, int n4){
        Random randomGen = new Random();
        for(int i = 0; i < n2; i++){
            SortedLinkedListSet<Integer> splayTree = new SortedLinkedListSet<>();
            TreeSet<Integer> treeSet = new TreeSet<>();
            for(int j = 0; j < n3; j++){
                double random = Math.random();
                if(random < 0.25){
